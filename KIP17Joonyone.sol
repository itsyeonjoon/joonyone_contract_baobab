...
// File: contracts\KIP17Joonyone.sol

pragma solidity ^0.5.0;
contract KIP17Joonyone is KIP17, KIP17Enumerable, KIP17Metadata, MinterRole {
    ...
    uint256[] private _minted;                     // list of TokenIDs that are already minted 
    ...
    /**
     * @dev gets base URI for the NFT artworks  
    ...
    /**
     * @dev sets URI to a given new base URI given as a parameter
     * @return void 
     */
    ...
    /**
     * @dev returns .json link associated to given token ID 
     * @return string memory 
     */
    function tokenURI(uint256 tokenId) public view returns (string memory)
    {
        require(
            0 < tokenId && tokenId <= _maxSaleAmount,
    ...
    /**
     * @dev checks if a given tokenId is already minted
     * @return bool
     */
    function isMinted(uint256 tokenId) public view returns (bool) {
        for (uint256 i = 0; i < _minted.length; i++) {
            if (tokenId == _minted[i]) {
                return true; 
            }
        }
        return false;  
    ...
    function mintingInformation() external view returns (uint256[4] memory){
      uint256[4] memory info =
        [_antibotInterval, _mintStartBlockNumber, _maxSaleAmount, _mintPrice];
      return info;
    }
    ...
    //Public Mint
    function publicMint(uint256 requestedIndex) external payable {  
      ...
      require(0 < requestedIndex && requestedIndex <= _maxSaleAmount, "token ID doesn't exist");
      require(msg.value == _mintPrice.mul(1), "Not enough Klay");
      require(_minted.length < _maxSaleAmount, "Minting is finished. Wait for new artworks to come!");
      require(balanceOf(msg.sender) + 1 <= _mintLimitPerSale, "Exceed max amount per person");


      require(!isMinted(requestedIndex));  
      _mint(msg.sender, requestedIndex);
      _minted.push(requestedIndex); 

      _lastCallBlockNumber[msg.sender] = block.number;
    }

    /**
     * @dev airdrops artwork to a user. 
     */
    function airDropMint(address user, uint256 requestedIndex) external onlyMinter {
      require(requestedIndex <= _maxSaleAmount && requestedIndex > 0, "token ID doesn't exist");
      require(!isMinted(requestedIndex)); 
      _mint(user, requestedIndex);
      _minted.push(requestedIndex); 
    }
}
 ...
 * @dev see http://kips.klaytn.com/KIPs/kip-17-non_fungible_token
 */
contract KIP17Full is KIP17, KIP17Enumerable, KIP17Metadata, Ownable, KIP17Joonyone {
...
// File: contracts\KIP17JoonyoneToken.sol
...
/**  
 * @title KIP17 Joonyone Token
 *
 * Credits to syl-doo2 for providing starter code for nft minting contract.
 * Also a special thanks to youtube-jocoding for giving inspiration to this minting site project. 
 */
contract KIP17JoonyoneToken is KIP17Full, KIP17Mintable, KIP17MetadataMintable, KIP17Burnable, KIP17Pausable {
